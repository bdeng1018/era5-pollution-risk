...
1. Run the **single‑variable download** explicitly:
...
1. Then run the pipeline stages via Makefile:
...
1. *(Optional)* Edit YAML configs in `configs/` (years, months, region, variables).

2. Download the single‑variable GRIB required for Branch 1:
...
1. Explore outputs, validate ingestion, inspect features and predictions.
